...
In the previous modules, we selected the **Granite-8b** model and calculated the tuning parameters (`max-model-len=8192`) required to make it run safely on our hardware. Now, we must deploy it.
...
---
[IMPORTANT]
.Extracting the official nvidia vLLM serving runtime?
====
Run this "One-Liner" 
It asks the cluster: "Hey, give me the official vLLM NVIDIA runtime configuration and install it in my namespace." where model-deploy-ab is the namespace.
+
[source,bash]
----
oc process vllm-cuda-runtime-template -n redhat-ods-applications | oc apply -f - -n model-deploy-lab
----

====

1. Get the Inference URL
Run this to grab the route directly from the KServe status:
...
2. Send the Request
Since vLLM provides an OpenAI-compatible API, we hit the /v1/chat/completions endpoint.

[source,bash]
----
curl -k -X POST "https://granite-3b-predictor.model-deploy-lab.svc.cluster.local:8443/v1/chat/completions" \
...
MODEL_PATH="granite3/" 
CONTEXT_LIMIT="16000"  # Tuned for Micro footprint
----
...
export URL=$(oc get inferenceservice granite3 -n my-first-model -o jsonpath='{.status.url}')
...
curl -k -X POST "https://$MODEL_URL/v1" \
