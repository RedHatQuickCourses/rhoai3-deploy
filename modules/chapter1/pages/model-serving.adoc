...
[source,bash]
.deploy and link the accounts.yaml
----
# 1. Apply the corrected secret
oc apply -f ./deploy/dataconn.yaml

# 2. Link it to the Service Account your inference service is using
# (Assuming your inference service uses 'fast-track-sa' based on previous steps)
...
Based on your feedback, we'll update this course accordingly: What type of model inference examples should be add to this course?
