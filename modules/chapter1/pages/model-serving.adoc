...
In the previous modules, we prepared the environment using the "Fast Track" script. Now, we must deploy the **Granite 3.3 2B** model using a reproducible KServe manifest.
...
curl -k -X POST "$MODEL_URL/v1/chat/completions" \
  ...
  -d '{
    ...
    "messages": [
      {
        "role": "user",
        "content": "Write a haiku about deploying AI on Kubernetes."
      }
    ],
    "temperature": 0.7,
    "max_tokens": 50
