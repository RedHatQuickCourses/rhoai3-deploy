...
Now, we're using the **granite-3.3-2b-instruct**, it's .75% smaller and faster experience.  So let's increase the max-model-len=16000, providing more KV cache to support larger prompts. 
...
=== **Create the Manifest:**
There is supplied file in the deploy folder called deploy.yaml.
...
spec:
  predictor:
    ...
    model:
      ...
      args:
        - "--max-model-len=16000"        # Limits context to 16k to save VRAM
