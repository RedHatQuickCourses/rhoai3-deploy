...
In the previous modules, we selected the **Granite-4-micro** model and calculated the tuning parameters (`max-model-len=16000`) required to make it run safely on our hardware.
...
Our script (`deploy/deploy-serve.sh`) automates the creation of two critical Kubernetes Custom Resources (CRs):
