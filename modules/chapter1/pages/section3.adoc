...
== 4. Debugging Model Loading (Registry Connection)

If your model is registered in the private registry but fails to load, verify the connection.

=== Check 1: Verify Model URI

The `model.uri` in your `LLMInferenceService` must match the URI format expected by your registry:
...
----
+
* **For Hugging Face:** Should be `hf://<model-name>`.
* **For Private Registry:** Should be `s3://<bucket>/<path>` or `oci://<registry>/<image>`.
...
=== Check 3: Check Model Registry Connection

If your model is in the private registry, verify the registry is accessible:

[source,bash]
----
oc get modelregistries -n rhoai-model-registries
----
+
* **Expected:** Your registry should be in `Ready` status.
